...
#define DIAG_WALL(fmt, args...)                                                \
    diag(DIAG_LEVEL_DEBUG, fmt, ##args);                                       \
    diag_wall(fmt, ##args)
...
#define DIAG_PRINT(fmt, args...)                                               \
    diag(DIAG_LEVEL_DEBUG, "[%04d@%s]" fmt, __LINE__, __func__, ##args)
#define DIAG_EMERG(fmt, args...)                                               \
    diag(DIAG_LEVEL_EMERG, BLINKFMT("[%04d@%s]" fmt), __LINE__, __func__,      \
         ##args)
#define DIAG_ERROR(fmt, args...)                                               \
    diag(DIAG_LEVEL_ERROR, REDFMT("[%04d@%s]" fmt), __LINE__, __func__, ##args)
#define DIAG_WARNING(fmt, args...)                                             \
    diag(DIAG_LEVEL_WARNING, YELLOWFMT("[%04d@%s]" fmt), __LINE__, __func__,   \
         ##args)
#define DIAG_INFO(fmt, args...)                                                \
    diag(DIAG_LEVEL_INFO, "[%04d@%s]" fmt, __LINE__, __func__, ##args)
#define DIAG_DEBUG(fmt, args...)                                               \
    diag(DIAG_LEVEL_DEBUG, "[%04d@%s]" fmt, __LINE__, __func__, ##args)
#define DIAG_VERBOSE(fmt, args...)                                             \
    diag(DIAG_LEVEL_VERBOSE, "[%04d@%s]" fmt, __LINE__, __func__, ##args)
#define DIAG_WALL(fmt, args...)                                                \
    diag(DIAG_LEVEL_DEBUG, "[%04d@%s]" fmt, __LINE__, __func__, ##args);       \
    diag_wall("[%04d@%s]" fmt, __LINE__, __func__, ##args)
#define DIAG_MEMORY(mem, size, fmt, args...)                                   \
    diag_memory(DIAG_LEVEL_DEBUG, mem, size, "[%04d@%s]" fmt, __LINE__,        \
                __func__, ##args)
#endif
