...
void diag_backtrace(void)
{
    ...
    if (size > 1) { /* skip the frame diag_backtrace itself. */
...
int diag_init(char *logname)
{
    ...
    g_diag_cfgfile = strappendfmt(&g_diag_cfgfile, "%s/%s.cfg", DIAG_ROOT_DIR, logname);
    g_diag_logfile = strappendfmt(&g_diag_logfile, "%s/%s.log", DIAG_ROOT_DIR, logname);
