...
int parse_cmdline(int argc, char** argv)
{
    ...
    app_t* app = app_create("food", PRJ_VERSION, PRJ_COPYRIGHT,
                            "liaoxf<liaofei1128@163.com>",
                            "food is a daemon program to control foo.");

    app_add_option(app, 'v', "dumpversion", NULL, 0,
                   "print the version string");
    app_add_option(app, 'f', "foreground", NULL, 0, "run food in foreground");
    app_add_option(app, 'D', "daemonize", NULL, 0,
                   "run the application as a daemon");

    app_add_cmdline(app, "dumpversion", do_dump_version,
                    "print the version string");
    app_add_cmdline(app, "daemonize", do_app_daemonize,
                    "run the application as a daemon");
    app_add_cmdline(app, "foreground", do_app_daemonize,
