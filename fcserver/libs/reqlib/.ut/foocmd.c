...
int main(int argc, char** argv)
{
    ...
    app_t* app =
        app_create("foocmd", PRJ_VERSION, PRJ_COPYRIGHT,
                   "gbj<gaobj021@163.com>,"
                   "liaoxf<liaofei1128@163.com>",
                   "foocmd is a program which can issue command to food.");

    app_add_option(app, 'f', "format", NULL, 0,
                   "do you want to format the output?");
    ...
    app_add_option(app, -1, FOO_COMMAND_TTY, NULL, 0,
                   "pass the tty name to food.");

    app_add_cmdline(app, FOO_COMMAND_ADD ",num,str,[format]", do_foo_add,
                    "foo add.");
    app_add_cmdline(app, FOO_COMMAND_DEL ",num,str,[format]", do_foo_del,
                    "foo del.");
    app_add_cmdline(app, FOO_COMMAND_TTY ",[format]", do_foo_tty,
                    "pass the tty name to food.");
