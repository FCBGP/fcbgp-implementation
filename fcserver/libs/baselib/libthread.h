...
static inline thread_t* thread_create(char* name, int flags,
                                      thread_handler_t handler, void* arg,
                                      int size)
{
    int ret;
    pthread_attr_t attr;
...
{
    thread_t* thread = thread_create((char*)__func__, THREAD_FLAG_DETACHED,
                                     handler, arg, size);
    if (thread)
    {
        while (1)
