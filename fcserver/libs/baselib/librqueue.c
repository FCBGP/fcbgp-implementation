...
void rqueue_clear(rqueue_t* rq) { rq->head = rq->tail = 0; }

void rqueue_destroy(rqueue_t* rq) { free(rq); }

int rqueue_capacity(rqueue_t* rq) { return rq->qsize - 1; }
...
int rqueue_isempty(rqueue_t* rq) { return (rq->tail == rq->head); }
...
int rqueue_foreach(rqueue_t* rq, int* cursor,
                   int (*handler)(int i, int idx, void* data, void* user),
                   void* user)
{
    int i, idx;
    int ret = -1;
...
int rqueue_foreach_reverse(rqueue_t* rq, int* cursor,
                           int (*handler)(int i, int idx, void* data,
                                          void* user),
                           void* user)
{
    int i, idx;
