...
int ncs_sock_sendto(int fd, void* data, int count, int* sent,
...
int ncs_sock_recvfrom(int fd, void* data, int count, int* got,
                      struct sockaddr* addr, socklen_t* len, int timeout)
{
    int ret = -1;
    int errcode = 0;
...
int ncs_buffer_sendto(int fd, void* data, int count, int* sent,
                      struct sockaddr* addr, socklen_t len, int timeout)
{
    int ret = -1;
    int txcnt = 0;
...
int ncs_buffer_recvfrom(int fd, void* data, int count, int* got,
                        struct sockaddr* addr, socklen_t* len, int timeout)
{
    int ret = -1;
    int rxlen = 0;
    ...
    *got = 0;
    ...
    {
        ret =
            ncs_sock_recvfrom(fd, dataptr, leftlen, &rxlen, addr, len, timeout);
        if (ret < 0)
        {
            *got = rxcnt;
            return ret;
...
static int ncs_server_init(ncs_ctx_t* ctx)
{
    ...
    ctx->server_sock =
        ncs_sock_create(PF_INET, ctx->is_udp ? SOCK_DGRAM : SOCK_STREAM, 0);
    ...
    DIAG_PRINT("ncs %s server try to bind %s:%d\n", ctx->name, ctx->local_addr,
               ctx->local_port);
    ret = ncs_sock_bind(ctx->server_sock, (struct sockaddr*)&server_sockaddr,
...
static int ncs_client_init(ncs_ctx_t* ctx)
{
    ...
    ctx->client_sock =
        socket(PF_INET, ctx->is_udp ? SOCK_DGRAM : SOCK_STREAM, 0);
...
int ncs_client_connect(ncs_ctx_t* ctx)
{
    ...
    ret = ncs_sock_connect(ctx->client_sock, (struct sockaddr*)&server_addr,
    ...
    while (1)
    {
        ...
        if (!ctx->is_udp)
        {
            DIAG_PRINT("ncs %s server try accept %d...\n", ctx->name,
                       ctx->server_sock);
            ctx->server_connid =
                ncs_sock_accept(ctx->server_sock, (struct sockaddr*)&cliaddr,
...
static int ncs_manager_init(ncs_ctx_t* ctx)
{
    ...
    if (pthread_create(&ctx->manager_tid, NULL, (void*)ncs_manager_handler,
                       (void*)ctx))
    {
...
void ncs_mutex_lock(ncs_ctx_t* ctx) { mutex_lock(&ctx->mutex); }

void ncs_mutex_unlock(ncs_ctx_t* ctx) { mutex_unlock(&ctx->mutex); }

int ncs_setup(ncs_ctx_t* ctx, char* local_addr, uint16_t local_port,
...
int ncs_client_send(ncs_ctx_t* ctx, char* data, int length)
{
    ...
    if (ctx->is_udp)
    {
        ctx->client_error = ncs_buffer_sendto(
            ctx->client_sock, data, length, &sent, &ctx->client_peeraddr,
            ctx->client_peerlen, ctx->send_timeout);
    }
    else
    {
        ctx->client_error = ncs_buffer_send(ctx->client_sock, data, length,
...
int ncs_client_recv(ncs_ctx_t* ctx, char* data, int length)
{
    ...
    if (ctx->is_udp)
    {
        ctx->client_error = ncs_buffer_recvfrom(
            ctx->client_sock, data, length, &got, &ctx->client_peeraddr,
            &ctx->client_peerlen, ctx->recv_timeout);
    }
    else
    {
        ctx->client_error = ncs_buffer_recv(ctx->client_sock, data, length,
...
int ncs_server_send(ncs_ctx_t* ctx, char* data, int length)
{
    ...
    if (ctx->is_udp)
    {
        ctx->server_error = ncs_buffer_sendto(
            ctx->server_connid, data, length, &sent, &ctx->server_peeraddr,
            ctx->server_peerlen, ctx->send_timeout);
    }
    else
    {
        ctx->server_error = ncs_buffer_send(ctx->server_connid, data, length,
                                            &sent, ctx->send_timeout);
    }
...
int ncs_server_recv(ncs_ctx_t* ctx, char* data, int length)
{
    ...
    if (ctx->is_udp)
    {
        ctx->server_error = ncs_buffer_recvfrom(
            ctx->server_connid, data, length, &got, &ctx->server_peeraddr,
            &ctx->server_peerlen, ctx->recv_timeout);
    }
    else
    {
        ctx->server_error = ncs_buffer_recv(ctx->server_connid, data, length,
                                            &got, ctx->recv_timeout);
    }
...
int ncs_server_register(ncs_ctx_t* ctx,
