...
int sys_arpinfo_get(sys_arpinfo_t** parpinfos)
{
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
        ...
        if (sscanf(buf, PROC_NET_ARP_FMT, arpinfo->ipaddr, &arpinfo->hwtype,
                   &arpinfo->flags, &arpinfo->hwaddr[0], &arpinfo->hwaddr[1],
                   &arpinfo->hwaddr[2], &arpinfo->hwaddr[3],
                   &arpinfo->hwaddr[4], &arpinfo->hwaddr[5], arpinfo->mask,
                   arpinfo->ifname) != 11)
        {
            free(arpinfo);
            continue;
