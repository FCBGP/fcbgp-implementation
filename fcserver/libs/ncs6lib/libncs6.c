...
int ncs6_sock_connect(int fd, struct sockaddr_in6 *addr, socklen_t len, int timeout)
{
    int ret = -1;
    int errcode = 0;
...
int ncs6_sock_accept(int fd, struct sockaddr_in6 *addr, socklen_t *len, int timeout)
{
    int ret= -1;
    int errcode = 0;
...
int ncs6_sock_sendto(int fd, void *data, int count, int *sent, struct sockaddr_in6 *addr, socklen_t len, int timeout)
{
    int ret = -1;
    int errcode = 0;
...
int ncs6_sock_recvfrom(int fd, void *data, int count, int *got, struct sockaddr_in6 *addr, socklen_t *len, int timeout)
{
    int ret = -1;
    int errcode = 0;
...
int ncs6_buffer_sendto(int fd, void *data, int count, int *sent, struct sockaddr_in6 *addr, socklen_t len, int timeout)
{
    int ret = -1;
    int txcnt = 0;
    int txlen = 0;
    ...
    *sent = 0;
    while (leftlen > 0) {
        ret = ncs6_sock_sendto(fd, dataptr, leftlen, &txlen, addr, len, timeout);
        if (ret < 0) {
            *sent = txcnt;
            return ret;
        }
...
int ncs6_buffer_recvfrom(int fd, void *data, int count, int *got, struct sockaddr_in6 *addr, socklen_t *len, int timeout)
{
    int ret = -1;
    int rxlen = 0;
    int rxcnt = 0;
    ...
    *got = 0;
    while (leftlen > 0) {
        ret = ncs6_sock_recvfrom(fd, dataptr, leftlen, &rxlen, addr, len, timeout);
        if (ret < 0) {
            *got = rxcnt;
            return ret;
        }
...
static int ncs6_server_init(ncs6_ctx_t *ctx)
{
    ...
    ctx->server_sock = ncs6_sock_create(AF_INET6, ctx->is_udp ? SOCK_DGRAM : SOCK_STREAM, 0);
    ...
    ret = ncs6_sock_bind(ctx->server_sock, (struct sockaddr_in6 *)&server_sockaddr, sizeof(struct sockaddr_in6));
    if (ret != 0) {
...
int ncs6_client_connect(ncs6_ctx_t *ctx)
{
    ...
    if (ctx->is_udp) {
        ...
        memcpy(&ctx->client_peeraddr, &server_addr, sizeof(struct sockaddr_in6));
        return 0;
    }

    ret = ncs6_sock_connect(ctx->client_sock, (struct sockaddr_in6 *) &server_addr, server_addr_length, ctx->recv_timeout);
    if (ret < 0) {
...
static int ncs6_server_start(ncs6_ctx_t *ctx)
{
    ...
    if (pthread_create(&ctx->server_tid, NULL, (void* (*)(void*)) ncs6_server_process, ctx)) {
    ...
    while (1) {
        ...
        if (ctx->serverinfo_changed) {
            if (ctx->server_started) {
                ...
                DIAG_DEBUG("ncs6 %s server existed, we will stop ncs6 server.\n", ctx->name);
...
static int ncs6_manager_init(ncs6_ctx_t *ctx)
{
    ...
    if (pthread_create(&ctx->manager_tid, NULL, (void* (*)(void*)) ncs6_manager_handler, (void *)ctx)) {
...
void ncs6_mutex_lock(ncs6_ctx_t *ctx)
{
    mutex_lock(&ctx->mutex);
}

void ncs6_mutex_unlock(ncs6_ctx_t *ctx)
{
    mutex_unlock(&ctx->mutex);
}

int ncs6_setup(ncs6_ctx_t *ctx, char *local_addr, uint16_t local_port, char *remote_addr, uint16_t remote_port)
{
    if (ctx == NULL) {
        return -EINVAL;
    }
...
int ncs6_client_send(ncs6_ctx_t *ctx, char *data, int length)
{
    ...
    if (ctx->is_udp) {
        ctx->client_error = ncs6_buffer_sendto(ctx->client_sock, data, length, &sent, &ctx->client_peeraddr, ctx->client_peerlen, ctx->send_timeout);
    } else {
        ctx->client_error = ncs6_buffer_send(ctx->client_sock, data, length, &sent, ctx->send_timeout);
    }
...
int ncs6_client_recv(ncs6_ctx_t *ctx, char *data, int length)
{
    ...
    if (ctx->is_udp) {
        ctx->client_error = ncs6_buffer_recvfrom(ctx->client_sock, data, length, &got, &ctx->client_peeraddr, &ctx->client_peerlen, ctx->recv_timeout);
    } else {
        ctx->client_error = ncs6_buffer_recv(ctx->client_sock, data, length, &got, ctx->recv_timeout);
    }
...
int ncs6_server_send(ncs6_ctx_t *ctx, char *data, int length)
{
    ...
    if (ctx->is_udp) {
        ctx->server_error = ncs6_buffer_sendto(ctx->server_connid, data, length, &sent, &ctx->server_peeraddr, ctx->server_peerlen, ctx->send_timeout);
    } else {
        ctx->server_error = ncs6_buffer_send(ctx->server_connid, data, length, &sent, ctx->send_timeout);
    }
...
int ncs6_server_recv(ncs6_ctx_t *ctx, char *data, int length)
{
    ...
    if (ctx->is_udp) {
        ctx->server_error = ncs6_buffer_recvfrom(ctx->server_connid, data, length, &got, &ctx->server_peeraddr, &ctx->server_peerlen, ctx->recv_timeout);
    } else {
        ctx->server_error = ncs6_buffer_recv(ctx->server_connid, data, length, &got, ctx->recv_timeout);
    }

    return got;
}

int ncs6_server_register(ncs6_ctx_t *ctx, int (*server_handler)(struct ncs6_ctx_st *ctx))
{
