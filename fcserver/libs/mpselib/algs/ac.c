 ...
 *
 * @see Alfred V. Aho and Margaret J. Corasick, Efficient String Matching: An
...
{
    uint16_t len;
    uint16_t offset0 : 1, offsetx : 1, priority : 5, res : 9;
    /* just save the origin string here, the lowercase one had been saved in the
     * state table. if not null, after AC matching, need to match with the it
     * again
     * */
    uint8_t* pattern;
    void* tag;
...
int build_ac_pattern_from_mpse_pattern(ac_alg_pattern_t* ac_pattern,
...
int ac_prepare_pattern_table(mpse_alg_ctx_t* ctx)
{
    ...
    priv->pattern_table =
        mpse_malloc(ctx->n_patterns * sizeof(ac_alg_pattern_t));
...
static inline int ac_state_set_output(mpse_alg_ctx_t* ctx, uint32_t state,
                                      mpse_pattern_t* pattern)
{
    ...
    output->n_pids++;
    output->pids = mpse_realloc(output->pids,
                                output->n_pids * sizeof(ac_alg_output_pid_t));
    if (output->pids == NULL)
    {
...
static inline int ac_state_club_output(mpse_alg_ctx_t* ctx, uint32_t dst_state,
                                       uint32_t src_state)
{
    int i = 0;
    int j = 0;
    ...
    for (i = 0; i < output_src_state->n_pids; i++)
    {
        ...
        if (j == output_dst_state->n_pids)
        {
            ...
            output_dst_state->pids = mpse_realloc(
                output_dst_state->pids,
                (output_dst_state->n_pids * sizeof(ac_alg_output_pid_t)));
            ...
            output_dst_pid =
                &output_dst_state->pids[output_dst_state->n_pids - 1];
...
static inline int ac_load_pattern(mpse_alg_ctx_t* ctx, mpse_pattern_t* pattern)
{
    ...
    {
        if (priv->goto_table[state][pattern->lowercase_pattern[i]] !=
            AC_STATE_FAIL)
        {
...
static inline int ac_create_failure_table(mpse_alg_ctx_t* ctx)
{
    ...
    while (state_fifo_dequeue(&fifo, &r_state))
    {
        for (ascii_code = 0; ascii_code < CHARSET_SIZE; ascii_code++)
        {
            ...
            priv->failure_table[temp_state] =
                priv->goto_table[state][ascii_code];
            ret = ac_state_club_output(ctx, temp_state,
                                       priv->failure_table[temp_state]);
            if (ret < 0)
            {
                state_fifo_fini(&fifo);
...
static inline int ac_create_delta_table(mpse_alg_ctx_t* ctx)
{
    ...
    {
        priv->state.u16_table =
            mpse_malloc(priv->n_states * sizeof(uint16_array_t));
        ...
        memset(priv->state.u16_table, 0,
               priv->n_states * sizeof(uint16_array_t));
            ...
            for (ascii_code = 0; ascii_code < CHARSET_SIZE; ascii_code++)
            {
                ...
                else
                {
                    priv->state.u16_table[r_state][ascii_code] =
                        priv->state.u16_table[priv->failure_table[r_state]]
                                             [ascii_code];
                }
            }
        }
    ...
    {
        priv->state.u32_table =
            mpse_malloc(priv->n_states * sizeof(uint32_array_t));
        ...
        memset(priv->state.u32_table, 0,
               priv->n_states * sizeof(uint32_array_t));
            ...
            for (ascii_code = 0; ascii_code < CHARSET_SIZE; ascii_code++)
            {
                ...
                else
                {
                    priv->state.u32_table[r_state][ascii_code] =
                        priv->state.u32_table[priv->failure_table[r_state]]
                                             [ascii_code];
                }
            }
        }
    ...
    for (state = 0; state < priv->n_states; state++)
    {
        for (ascii_code = 0; ascii_code < CHARSET_SIZE; ascii_code++)
        {
            ...
            if (priv->output_table[temp_state & 0x7FFFFFFF].n_pids != 0)
                priv->state.u32_table[state & 0x7FFFFFFF][ascii_code] |=
                    0x80000000;
        }
    }
...
static int ac_alg_search(mpse_alg_ctx_t* ctx, uint8_t* text, uint16_t len,
        ...
        for (i = 0; i < len; i++)
        {
            ...
            if (priv->output_table[state].n_pids != 0)
            {
                ...
                for (k = 0; k < n_pids; k++)
                {
                    ...
                    if (pids[k].nocase == 0)
                    {
                        if (memcmp(pattern->pattern, text + start,
                                   pattern->len))
                        {
                            continue;
                        }
                    ...
                    n_matchers = mpse_matchers_output(
                        matchers, n_matchers, pids[k].priority, pids[k].pid,
                        start, i, pattern->tag);
                }
            }
        }
...
int ac_alg_memstat(mpse_alg_ctx_t* ctx)
{
    ...
    if (priv)
    {
        ...
        if (priv->output_table)
        {
            ...
            {
                memstat +=
                    priv->output_table[i].n_pids * sizeof(ac_alg_output_pid_t);
            }
        }
