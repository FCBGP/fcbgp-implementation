...
typedef struct mpse_pattern_st
{
    ...
    uint8_t* lowercase_pattern; /** the lowercase pattern string, if same with
                                   pattern, then use the pointer of pattern */
    void* tag;
} mpse_pattern_t;
...
static inline int mpse_matchers_init(mpse_matchers_t* matchers,
                                     int max_matchers, int allow_repeat,
                                     int allow_priority)
{
    int n;
    int i, j;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        matchers->classes[i].res =
            mpse_malloc(max_matchers * sizeof(mpse_matcher_t));
        ...
        memset(matchers->classes[i].res, 0,
               max_matchers * sizeof(mpse_matcher_t));
    }
...
static inline int mpse_matchers_output(mpse_matchers_t* matchers,
                                       int n_matchers, int priority,
                                       uint32_t pid, uint16_t start,
                                       uint16_t end, void* tag)
{
    mpse_matcher_class_t* classes;
    ...
    if (matchers)
    {
        ...
        if (classes)
        {
            ...
            {
                if (matchers->allow_repeat ||
                    !mpse_matchers_exist(classes->n, classes->res, pid))
                {
...
typedef struct mpse_alg_st
{
    ...
    int (*alg_search)(mpse_alg_ctx_t* ctx, uint8_t* text, uint16_t len,
...
extern int mpse_add(void* mpse, uint16_t priority, uint8_t* pattern,
                    uint16_t len, uint16_t flags, uint32_t* pid, void* tag);
extern int mpse_prepare(void* mpse);
extern int mpse_search(void* mpse, uint8_t* text, uint16_t text_len,
                       mpse_matchers_t* matchers, uint16_t flags);
