...
/*
 * args: {proto, timeout, rcvbuf, sndbuf, mtu, promisc, padding_begin,
 * padding_end}
 */
int raw_sock_open(nio_ctx_t* ctx, int argc, int* argv)
{
    ...
    if (bind(priv->sock, (struct sockaddr*)&priv->address,
             sizeof(priv->address)) < 0)
    {
        close(priv->sock);
        free(priv);
        return -EADDRINUSE;
    ...
    setsockopt(priv->sock, SOL_SOCKET, SO_RCVBUF,
               (const char*)&priv->rcvbuf_size, sizeof(int));
    setsockopt(priv->sock, SOL_SOCKET, SO_SNDBUF,
               (const char*)&priv->sndbuf_size, sizeof(int));
...
int raw_sock_read(raw_sock_priv_t* priv, uint8_t** pdata)
{
    ...
    while (1)
    {
        rxlen = recvfrom(priv->sock, data + offset, blocksize, MSG_DONTWAIT,
                         NULL, NULL);
        if (rxlen < 0)
        {
            free(data);
...
int raw_sock_send(nio_ctx_t* ctx, uint16_t proto, uint8_t* data, int len)
{
    ...
    memcpy(buffer, ctx->dmac, 6);
    offset += 6;
    memcpy(buffer + offset, ctx->smac, 6);
    offset += 6;
    *(buffer + offset) = ((proto >> 8) & 0xFF);
    offset += 1;
    *(buffer + offset) = (proto & 0xFF);
    offset += 1;
    ...
    ret =
        sendto(priv->sock, buffer, length, MSG_DONTWAIT,
               (struct sockaddr*)(&priv->address), sizeof(struct sockaddr_ll));
    if (ret != length)
    {
...
int raw_sock_forward(nio_ctx_t* ctx, uint16_t proto, nio_pkt_t* pkt)
{
    ...
    ret =
        sendto(priv->sock, pkt->data, pkt->length, MSG_DONTWAIT,
               (struct sockaddr*)(&priv->address), sizeof(struct sockaddr_ll));
    if (ret != pkt->length)
    {
