...
static uint8_t g_scanner_smac[6] = {
    0,
};
...
int reply_process(nio_pkt_t* pkt) { return 0; }

int relay_reply_process(nio_pkt_t* pkt)
{
    //   int ret = 0;
    //   dhcpv6_relay_t *dr6;
    //   dr6 = parse_relay_dhcp((unsigned char *)pkt+62, pkt->length);
    //   if (dr6 == NULL) {
    ...
    //   value1 = &tmp1;
    //
    //   ret = get_option(dr6->opt_list, 0x09, value);
    //
    //   dhcpv6_t *dhc6;
    //   dhc6 = parse_dhcp((unsigned char *)*value, ret);
    //   if (dhc6 == NULL) {
    //       DIAG_ERROR("parse dhcp failed.\n");
    //       return -1;
    //   }
    //
    //   ret = get_option(dhc6->opt_list, 0x03, value1);
    //
    //   parse_dhcp_relay_fini(dr6);
...
int scanner_process_ipv6(nio_ctx_t* ctx, nio_pkt_t* pkt)
{
    ...
    req_msg = NULL; // we should not to free it, because nio_forward
    return 0;
}
...
int scanner_link_init(char* ifname)
{
    ...
    g_scanner_nio =
        nio_create(ifname, g_scanner_smac, g_scanner_dmac, &g_raw_sock_ops);
    if (g_scanner_nio == NULL)
    {
...
{
    int ret;
    char tmp[NIO_SCANNER_LENGTH + 8] = {
        0,
    };
    char buffer[NIO_SCANNER_LENGTH + 8] = {
        0,
    };
    IP_HEADER* ip = NULL;
    ...
    tcpHeader.th_sum = htons(my_checksum(
        (unsigned char*)tmp, sizeof(psdHeader) + sizeof(tcpHeader)));

    //最终的组包（TCP+IP）
    memcpy(buffer + sizeof(IP_HEADER), &tcpHeader, sizeof(tcpHeader));

    ret = nio_send(g_scanner_nio, PROTO_IP, (uint8_t*)buffer,
                   sizeof(IP_HEADER) + sizeof(tcpHeader));
    if (ret < 0)
    {
...
int myscanner_send_hex(char* hex)
{
    ...
    buffer = NULL; // we should not use free, because it is freed in nio_forward
                   // function
    return 0;
}
...
int mysql_postauth(char* username, char* mac)
{
    ...
    snprintf(sql_cmd, 1024,
             "insert into radpostauth(username, pass, reply, user_mac) values "
             "(\'%s\', 0, 'Access-Accept', \'%s\')",
             username, mac);
    ...
    if (mysql_real_connect(conn, MYSQL_IP, MYSQL_USER, MYSQL_PASS,
...
void* process_solicit(void* arg)
{
    int i;
    int ret = 0;
    dhcpv6_msg_t* msg = (dhcpv6_msg_t*)arg;

    char* ifname = msg->ifname;
    int count = msg->count;
    char loop = msg->loop;
...
#if 1
        ret = relay_solicit(LOCAL_IPV6, REMOTE_IPV6, LINK_ADDR, smac_hex,
                            ifname, solicit_msg);
        if (ret < 0)
        {
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        {
            printf("%s -i <interface> { -syn <destip> | -hex <hexcode> | -dhcp "
                   "<count> } -v | -h | -loop \n",
                   argv[0]);
            return 0;
        }
    ...
    if (dhcp_test)
    {
        ...
        uint64_t client_mac = 0x00;
        pthread_t t[4] = {0};
        dhcpv6_msg_t msg;
        memset(&msg, 0, sizeof(dhcpv6_msg_t));
        msg.ifname = ifname;
        msg.count = count;
        msg.loop = loop;
        for (i = 0; i < 4; i++)
        {
            pthread_create(&t[i], NULL, (void*)process_solicit, (void*)&msg);
        }
    }
