...
bool mpack_write_ext16(mpack_ctx_t * ctx, int8_t tp, uint16_t sz, const void *data)
{
...
bool mpack_write_ext32(mpack_ctx_t * ctx, int8_t tp, uint32_t sz, const void *data)
{
...
bool mpack_write_object(mpack_ctx_t * ctx, mpack_object_t * obj)
{
    ...
    case MPACK_TYPE_EXT8:
        return mpack_write_ext8_marker(ctx, obj->as.ext.type, (uint8_t) obj->as.ext.size);
    case MPACK_TYPE_EXT16:
        return mpack_write_ext16_marker(ctx, obj->as.ext.type, (uint16_t) obj->as.ext.size);
    case MPACK_TYPE_EXT32:
        return mpack_write_ext32_marker(ctx, obj->as.ext.type, obj->as.ext.size);
...
static void mpack_object_suffix(stream_t *output, int formated, int iskey, int islast)
{
    if (iskey) {
        stream_printf(output, ":");
    } else {
        if (!islast) stream_printf(output, ",");
        if (formated) stream_printf(output, "\n");
    }
}

static void mpack_ctx_dump_internal(stream_t *output, mpack_ctx_t *ctx, int formated, int depth, int iskey, int islast)
{
    int i;
    mpack_object_t obj;
    ...
    case MPACK_TYPE_MAP16:
    case MPACK_TYPE_MAP32:
        if (formated) mpack_object_indent(output, depth);
        stream_printf(output, "{");
        if (formated) stream_printf(output, "\n");
        for (i=0; i<obj.as.map_size; i++) {
            mpack_ctx_dump_internal(output, ctx, formated, depth + 1, 1, 0);
            mpack_ctx_dump_internal(output, ctx, formated, depth + 1, 0, i == obj.as.map_size - 1);
        }
        if (formated) mpack_object_indent(output, depth);
        stream_printf(output, "}");
    ...
    case MPACK_TYPE_ARRAY32:
        ...
        for (i=0; i<obj.as.array_size; i++) {
            mpack_ctx_dump_internal(output, ctx, formated, depth + 1, 0, i == obj.as.array_size - 1);
        }
        if (formated) mpack_object_indent(output, depth);
        stream_printf(output, "]");
        mpack_object_suffix(output, formated, iskey, islast);
    ...
    case MPACK_TYPE_STR32:
        {
            ...
            if (formated && iskey) mpack_object_indent(output, depth);
            stream_printf(output, "\"");
    ...
    case MPACK_TYPE_UINT64:
        if (formated && iskey) mpack_object_indent(output, depth);
        stream_printf(output, formated ? "0x%"PRIX64 : "%"PRIu64, obj.as.u64);
        mpack_object_suffix(output, formated, iskey, islast);
        break;
    case MPACK_TYPE_NEGATIVE_FIXNUM:
    case MPACK_TYPE_SINT8:
...
void mpack_ctx_dump(stream_t *output, mpack_ctx_t *ctx, int formated)
{
    ...
    if (!formated) stream_printf(dumpoutput, "\n");
