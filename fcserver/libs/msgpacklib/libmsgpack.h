...
typedef bool (*mpack_reader_t)(struct mpack_ctx_st* ctx, void* data,
                               size_t limit);
typedef size_t (*mpack_writer_t)(struct mpack_ctx_st* ctx, const void* data,
                                 size_t count);
...
bool mpack_write_str_for_snmp(mpack_ctx_t* ctx, const char* data,
                              uint32_t size);
...
bool mpack_write_ext(mpack_ctx_t* ctx, int8_t type, uint32_t size,
                     const void* data);
...
bool mpack_write_ext8(mpack_ctx_t* ctx, int8_t type, uint8_t size,
                      const void* data);
bool mpack_write_ext16_marker(mpack_ctx_t* ctx, int8_t type, uint16_t size);
bool mpack_write_ext16(mpack_ctx_t* ctx, int8_t type, uint16_t size,
                       const void* data);
bool mpack_write_ext32_marker(mpack_ctx_t* ctx, int8_t type, uint32_t size);
bool mpack_write_ext32(mpack_ctx_t* ctx, int8_t type, uint32_t size,
                       const void* data);
...
static inline bool mpack_stream_reader(mpack_ctx_t* ctx, void* data,
                                       size_t size)
{
    return stream_read((stream_t*)ctx->buf, (char*)data, size) == size;
}

static inline size_t mpack_stream_writer(mpack_ctx_t* ctx, const void* data,
                                         size_t size)
{
