...
#define LISTEN_EVENT_RWT                                                       \
    (LISTEN_EVENT_READ | LISTEN_EVENT_WRITE | LISTEN_EVENT_TIMEOUT)
...
struct listener_st
{
    ...
    // 0:  may be removed by the callback, but will not be freed  until callback
    // returns >0: has been removed, and should be freed when the callback
    // returns
    int delayFree;

    // Number of seconds after which this listener will receive a timeout
    // message
    // (-1 means no timeout).
    time_t timeout;

    // real time when this listener will receive a timeout message (set to
    // timeout
    // + current time).
    time_t expiration;
...
typedef struct listenerset_st
{
    ...
    // Minimum amount of time we should wait in select based on the smallest
    // time left for any listener in this set.
    time_t minExpiration;

    // Linked list of all the listeners in this set.
    struct list_head list;

    // Callback to be called if it is desired to be notified of interrupted
    // system calls. This value can be NULL.
...
extern int listenerset_enumerate(listenerset_t* set,
                                 listener_enumerator_t* enumerator, void* arg);
extern listener_t* listenerset_add(listenerset_t* set, int fd,
                                   listener_handler_t* handler,
                                   listen_event_t type, void* tag,
                                   time_t timeout);
#define listenerset_addtimer(set, handler, tag, timeout)                       \
    listenerset_add(set, -1, handler, LISTEN_EVENT_TIMEOUT, tag, timeout)
extern int listenerset_update_remover(listenerset_t* set,
                                      listener_remover_t* remover);
extern int listenerset_update_interrupter(listenerset_t* set,
                                          listener_interrupter_t* interrupter,
                                          void* arg);
...
extern int listener_set_handler(listener_t* listener,
                                listener_handler_t* handler);
