...
static void do_child_process(void)
{
    ...
    for (i = 0; i < NUM_PIPES; i++) {
        ...
        listenerset_add(&g_listener_set, pipevs[i][0], listener_handler,
            LISTEN_EVENT_READ | LISTEN_EVENT_TIMEOUT | LISTEN_EVENT_PREEMPT | LISTEN_EVENT_MARKED, NULL, 1);
    }
...
void cut_define(void *ctx)
{
    ...
    cut_add_test(ctx, 1, "main_test", main_test, NULL, "listener test main entry");
}
