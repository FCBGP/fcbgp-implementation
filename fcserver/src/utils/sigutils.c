...
static inline size_t fc_calc_base64_decode_len(const char* b64msg)
{
...
static int fc_sha_encode(const unsigned char* const msg, int msglen,
                         unsigned char* digest, unsigned int* digest_len,
                         const char* sha_hash_algo)
{
    int ret = 1;
    EVP_MD* md = NULL;
...
static int fc_md5_encode(const unsigned char* const msg, int msglen,
                         unsigned char* digest, unsigned int* digest_len)
{
    return fc_sha_encode(msg, msglen, digest, digest_len, "MD5");
}

static int fc_sha1_encode(const unsigned char* const msg, int msglen,
                          unsigned char* digest, unsigned int* digest_len)
{
    return fc_sha_encode(msg, msglen, digest, digest_len, "SHA1");
}

static int fc_sha256_encode(const unsigned char* const msg, int msglen,
                            unsigned char* digest, unsigned int* digest_len)
{
    return fc_sha_encode(msg, msglen, digest, digest_len, "SHA256");
}

static int fc_get_ski(X509* cert, u8* ski, int* skilen)
{
...
int fc_get_ecpubkey_and_ski(u32 asn, const char* fpath, EC_KEY** ecpubkey,
                            u8* ecski)
{
    X509* cert = NULL;
...
int fc_read_eckey_from_file(const char* fpath, FC_KEY_TYPE key_type,
                            EC_KEY** pkey)
{
    FILE* fp = NULL;
...
static int fc_hash(const unsigned char* const msg, int msglen,
                   unsigned char* digest, unsigned int* digestlen)
{
    ...
    // sts.tv_nsec); DIAG_INFO("\e[31mHASH FUNC TIME END   %ld.%09ld s\e[0m\n",
    // ets.tv_sec, ets.tv_nsec); DIAG_INFO("\e[31mTIME SPENT IN HASH   %ld.%09ld
    // s\e[0m\n", tvsec, tvnsec);
    return 0;
}
