...
#include <string.h>
    ...
    static int fc_acl_gen_linux(int clisockfd, const FC_msg_bm_t* bm)
    {
        int i = 0, j = 0, ret = 0;
        ...
        inet_ntop(AF_INET,
                  &(((struct sockaddr_in*)&(bm->dst_ip[0].ip))->sin_addr),
        ...
        for (i = 0; i < bm->src_ip_num; ++i)
        {
            ...
            {
                for (j = FC_NFT_FILTER_CHAIN_START; j < FC_NFT_FILTER_CHAIN_END;
                     ++j)
                {
                    sprintf(cmd,
                            "nft add rule inet filter %s "
                            "ip saddr %s/%d ip daddr %s/%d drop",
                            g_fc_nft_chains[j], saddr,
                            bm->src_ip[0].prefix_length, daddr,
                            bm->dst_ip[0].prefix_length);
                    ret = system(cmd);
            ...
            else // filter: !a->d
            {
                for (j = 0; j < g_fc_server.nics_num; ++j)
                {
                    if (strcmp(g_fc_server.nics[j], ifname))
                    {
                         ...
                         *      ip saddr 192.168.88.131 ip daddr 192.168.88.132
                         * drop
                         * */
    ...
    static int fc_acl_gen_h3c(int clisockfd, const FC_msg_bm_t* bm)
    {
        ...
        direction_e direction =
            0;           // 0 for none, 1 for in, 2 for out, 3 for both
        ...
        for (int i = 0; is_still_loop && i < bm->dst_ip_num; ++i)
        {
            switch (bm->ipversion)
            {
                ...
                case IPV6:
                    inet_ntop(AF_INET6,
                              &(((struct sockaddr_in6*)&(bm->dst_ip[i].ip))
                                    ->sin6_addr),
                              daddr, (socklen_t)sizeof(daddr));
                    break;
            }
            ...
            for (int j = 0; is_still_loop && j < bm->src_ip_num; ++j)
            {
                switch (bm->ipversion)
                {
                    case IPV4:
                        inet_ntop(AF_INET,
                                  &(((struct sockaddr_in*)&(bm->src_ip[j].ip))
                                        ->sin_addr),
                                  saddr, (socklen_t)sizeof(saddr));
                        break;
                    case IPV6:
                        inet_ntop(AF_INET6,
                                  &(((struct sockaddr_in6*)&(bm->src_ip[j].ip))
                                        ->sin6_addr),
                                  saddr, (socklen_t)sizeof(saddr));
                        break;
                }
                ...
                if (router_info == NULL)
                {
                    ...
                    DIAG_ERROR(
                        "Have you configured it in configuration file: %s?\n",
                ...
                while (router_info)
                {
                    ...
                    {
                        DIAG_ERROR(
                            "Router (bgpid: %08X) does not hold any link.\n",
                            router_info->bgpid);
                        is_still_loop = false;
                    }
                    ...
                    while (link_info)
                    {
                        for (iface_info = link_info->iface_list;
                             iface_info != NULL; iface_info = iface_info->next)
                        {
                            ...
                            {
                                DIAG_INFO(
                                    "offpath node srcip: %s/%d, dstip: %s/%d, "
                                    "iface_index: %08X, direction: both\n",
                                    saddr, sprefixlen, daddr, dprefixlen,
                                    iface_info->iface_index);
                                ...
                                else // update
                                {
                                    ...
                                    DIAG_INFO(
                                        ...
                                        __FUNCTION__, __LINE__,
                                        item->acl_group_index, bm->ipversion,
                                        rule_id, saddr, sprefixlen, daddr,
                                        dprefixlen, iface_info->iface_index);
                                    py_apply_acl(&router_info->py_config,
                                                 item->acl_group_index,
                                                 bm->ipversion, is_withdraw,
                                                 rule_id, saddr, sprefixlen,
                                                 daddr, dprefixlen,
                                                 iface_info->iface_index,
                            ...
                            else // onpath node
                            {
                                ...
                                else
                                {
                                    ...
                                    // this is for the other ifaces that do not
                                    // link to nasn or pasn.
                                    else
                                    {
                                        direction = FC_TOPO_DIRECTION_NONE;
                                        direction_str = "none";
                                ...
                                DIAG_INFO(
                                    "onpath node srcip: %s/%d, dstip: %s/%d, "
                                    "iface_index: %08X, direction: %s\n",
                                    saddr, sprefixlen, daddr, dprefixlen,
                                    iface_info->iface_index, direction_str);
                                if (direction == FC_TOPO_DIRECTION_NONE)
                                ...
                                if (direction & FC_TOPO_DIRECTION_IN)
                                {
                                    if (is_withdraw) // withdraw/del
                                    {
                                        ht_acl_rule_info_t* acl_rule_info =
                                            NULL;
                                        ht_acl_rule_info_t* rule =
                                            (ht_acl_rule_info_t*)calloc(
                                                1, sizeof(ht_acl_rule_info_t));
                                        rule->ipversion = bm->ipversion;
                                        memcpy(rule->saddr, saddr,
                                               INET6_ADDRSTRLEN);
                                        rule->sprefixlen = sprefixlen;
                                        memcpy(rule->daddr, daddr,
                                               INET6_ADDRSTRLEN);
                                        rule->dprefixlen = dprefixlen;
                                        rule->rule_id = 0;
                                        rule->direction =
                                            FC_TOPO_DIRECTION_NONE;
                                        rule->acl_group_index =
                                            item->acl_group_index;
                                        acl_rule_info_key =
                                        ...
                                        if (acl_rule_info)
                                        {
                                            DIAG_INFO(
                                                ...
                                                item->acl_group_index,
                                                bm->ipversion,
                                                acl_rule_info->rule_id, saddr,
                                                sprefixlen, daddr, dprefixlen,
                                                iface_info->iface_index);
                                            py_apply_acl(
                                                &router_info->py_config,
                                                item->acl_group_index,
                                                bm->ipversion, is_withdraw,
                                                acl_rule_info->rule_id, saddr,
                                                sprefixlen, daddr, dprefixlen,
                                                iface_info->iface_index,
                                                FC_TOPO_DIRECTION_IN);
                                            HASH_DEL(item->ht_acl_rule_info,
                                                     acl_rule_info);
                                            free(acl_rule_info);
                                        }
                                        else
                                        {
                                            DIAG_INFO(
                                                "no such onpath withdraw "
                                                "acl-in direction, "
                                                "[%s:%d] group index: %u, "
                                                "ipversion: %d fake rule_id: "
                                                "%u, "
                                                "src prefix: %s/%d, dst "
                                                "prefix: %s/%d, "
                                                "iface_index: %08X\n",
                                    ...
                                    else // update/add
                                    {
                                        rule_id = ++item->acl_rule_in_id;
                                        DIAG_INFO(
                                            "onpath update acl-in direction, "
                                            "[%s:%d] group index: %u, "
                                            "ipversion: %d rule_id: %u, "
                                            "src prefix: %s/%d, dst prefix: "
                                            "%s/%d, "
                                            "iface_index: %08X\n",
                                            __FUNCTION__, __LINE__,
                                            item->acl_group_index,
                                        ...
                                        acl_rule_info->ipversion =
                                            bm->ipversion;
                                        memcpy(acl_rule_info->saddr, saddr,
                                               INET6_ADDRSTRLEN);
                                        acl_rule_info->sprefixlen = sprefixlen;
                                        memcpy(acl_rule_info->daddr, daddr,
                                               INET6_ADDRSTRLEN);
                                        acl_rule_info->dprefixlen = dprefixlen;
                                        acl_rule_info->rule_id = 0;
                                        ...
                                        acl_rule_info->acl_group_index =
                                            item->acl_group_index;
                                        acl_rule_info_key =
                                            fnv1a_hash(acl_rule_info,
                                                       sizeof(acl_rule_info));
                                        acl_rule_info->rule_id = rule_id;
                                        acl_rule_info->direction =
                                            FC_TOPO_DIRECTION_IN;
                                ...
                                if (direction & FC_TOPO_DIRECTION_OUT)
                                {
                                    ...
                                    else
                                    {
                                        rule_id = ++item->acl_rule_out_id;
                                        DIAG_INFO(
                                            "onpath update acl-out direction, "
                                            "[%s:%d] group index: %u, "
                                            "ipversion: %d rule_id: %u, "
                                            "src prefix: %s/%d, dst prefix: "
                                            "%s/%d, "
