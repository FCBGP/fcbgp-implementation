...
static wm_alg_bloomfilter_t *wm_alg_bloomfilter_init(uint32_t size, uint8_t iter, uint32_t(*Hash) (void *, uint16_t, uint8_t, uint32_t))
{
...
static int wm_alg_bloomfilter_add(wm_alg_bloomfilter_t * bf, void *data, uint16_t datalen)
{
    uint8_t iter = 0;
    uint32_t hash = 0;
...
static inline int wm_alg_bloomfilter_test(wm_alg_bloomfilter_t * bf, void *data, uint16_t datalen)
{
    uint8_t iter = 0;
    uint32_t hash = 0;
    int hit = 1;
...
typedef struct wm_alg_ctx_st {
    ...
    uint32_t (*Search)(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags);
    uint32_t (*MBSearch)(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags);
} wm_alg_ctx_t;
...
static inline void wm_alg_free_pattern(mpse_alg_ctx_t * mpm_ctx, wm_alg_pattern_t * p)
{
    if (p && p->cs && p->cs != p->ci) {
        mpse_free(p->cs);
    }
...
static int build_wm_pattern_from_mpse_pattern(wm_alg_pattern_t * wm_pattern, mpse_pattern_t * mpse_pattern)
{
    ...
    mpse_memcpy_nocase(wm_pattern->ci, mpse_pattern->pattern, mpse_pattern->len);

    if (wm_pattern->nocase) {
        wm_pattern->cs = wm_pattern->ci;
    } else {
        if (memcmp(wm_pattern->ci, mpse_pattern->pattern, mpse_pattern->len) == 0) {
            wm_pattern->cs = wm_pattern->ci;
        } else {
    ...
    if (wm_pattern->len > 1) {
        wm_pattern->prefix_cs = (uint16_t) (*(wm_pattern->cs) + *(wm_pattern->cs + 1));
        wm_pattern->prefix_ci = (uint16_t) (*(wm_pattern->ci) + *(wm_pattern->ci + 1));
    }
...
static int wm_alg_prepare_pattern_array(mpse_alg_ctx_t * mpm_ctx)
{
    ...
    ctx->parray = (wm_alg_pattern_t **) mpse_malloc(ctx->n_patterns * sizeof(wm_alg_pattern_t *));
    if (ctx->parray == NULL) {
        return -ENOMEM;
    }
    ...
    for (i = 0; i < mpm_ctx->n_patterns; i++) {
        ...
        if (ctx->minlen == 0 || ((ctx->minlen != 0) && (ctx->minlen > wm_pattern->len))) {
...
static uint32_t wm_alg_bloomfilter_func(void *data, uint16_t datalen, uint8_t iter, uint32_t hash_size)
{
...
static int wm_alg_prepare_hashTable(mpse_alg_ctx_t * mpm_ctx)
{
    ...
    ctx->hash = (wm_alg_hashitem_t **) mpse_malloc(sizeof(wm_alg_hashitem_t *) * ctx->hash_size);
    if (ctx->hash == NULL) {
        return -ENOMEM;
    }
    ...
    ctx->bloom = (wm_alg_bloomfilter_t **) mpse_malloc(sizeof(wm_alg_bloomfilter_t *) * ctx->hash_size);
    if (ctx->bloom == NULL) {
        mpse_free(ctx->pminlen);
        for (j = 0; j < 256; j++)
            wm_alg_free_hashitem(mpm_ctx, ctx->hash1[j].nxt);
    ...
    for (h = 0; h < ctx->hash_size; h++) {
        ...
        ctx->bloom[h] = wm_alg_bloomfilter_init(wm_bloom_size, 2, wm_alg_bloomfilter_func);
        if (ctx->bloom[h] == NULL) {
            for (j = 0; j < h; j++)
        ...
        do {
            wm_alg_bloomfilter_add(ctx->bloom[h], ctx->parray[thi->idx]->ci, ctx->pminlen[h]);
            thi = thi->nxt;
        } while (thi != NULL);
    }
...
static uint32_t wm_alg_search2hash9(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
    uint32_t cnt = 0;
            ...
            if (hi != NULL) {
                if (ctx->bloom[h] != NULL) {
                    if ((bufend - (buf - sl)) < ctx->pminlen[h]) {
                        ...
                        if (wm_alg_bloomfilter_test(ctx->bloom[h], buf - sl + 1, ctx->pminlen[h]) == 0) {
                            goto skip_loop;
                        }
                    }
                }
                ...
                for (thi = hi; thi != NULL; thi = thi->nxt) {
                    ...
                    if (p->nocase) {
                        ...
                        if (mpse_memcmp_nocase(p->ci, buf - sl + 1, p->len) == 0) {
                            ...
                            if (p->offsetx)
                                if (end != buflen - 1)
                                    continue;
                            cnt = mpse_matchers_output(matchers, cnt, p->priority, p->pid, start, end, p->tag);
                        }
                    } else {
...
static uint32_t wm_alg_search2hash12(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
    uint32_t cnt = 0;
    uint8_t *bufend = buf + buflen - 1;
...
static uint32_t wm_alg_search2hash14(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
    uint32_t cnt = 0;
    uint8_t *bufend = buf + buflen - 1;
...
static uint32_t wm_alg_search2hash15(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
    uint32_t cnt = 0;
    uint8_t *bufend = buf + buflen - 1;
...
static uint32_t wm_alg_search2hash16(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
    uint32_t cnt = 0;
...
static uint32_t wm_alg_search1(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
    uint8_t *bufstart = buf;
    uint8_t *bufend = buf + buflen - 1;
...
static int wm_alg_search(mpse_alg_ctx_t * mpm_ctx, uint8_t * buf, uint16_t buflen, mpse_matchers_t * matchers, uint16_t flags)
{
    wm_alg_ctx_t *ctx = (wm_alg_ctx_t *) mpm_ctx->data;
        ...
        if (ctx->parray) {
            ...
            for (i = 0; i < ctx->n_patterns; i++) {
                ...
                if (temp_pattern) {
                    ...
                    if (temp_pattern->cs && (temp_pattern->cs != temp_pattern->ci))
                        memstat += temp_pattern->len * sizeof(uint8_t);
                }
            }
        }
