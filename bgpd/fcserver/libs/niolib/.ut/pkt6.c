...
unsigned char * ClientId(char *smac, int *size){//cliId occupies 18 bytes
...
dhcpv6_relay_t* parse_relay_dhcp(unsigned char *dhcp, int len)
{
    ...
    dhcpv6_relay_pkt->msgtype = *p; p+=1; len-=1;
    dhcpv6_relay_pkt->hopcount = *p; p+=1; len-=1;
    memcpy(dhcpv6_relay_pkt->link_addr, p, 16); p+=16; len-=16;
    memcpy(dhcpv6_relay_pkt->peer_addr, p, 16); p+=16; len-=16;

    dhcpv6_opt_t *opt;

    while (len > 0) {
        opt = malloc(sizeof(dhcpv6_opt_t));
        opt->code = *p<<8 | *(p+1); p+=2; len-=2;
        opt->len = *p<<8 | *(p+1); p+=2; len-=2;
        opt->value = malloc(opt->len+1);
        memcpy(opt->value, p, opt->len); p+=opt->len; len-=opt->len;
...
dhcpv6_t* parse_dhcp(unsigned char *dhcp, int len)
{
    ...
    dhcpv6_pkt->msgtype = *dhcp; p+=1; len-=1;
    memcpy(dhcpv6_pkt->transID, p, 3); p+=3; len-=3;
...
static int add_request_option(char *addr)
{
    ...
    char option_request[32] = {0x00, 0x11, 0x00, 0x17, 0x00, 0x18, 0x00, 0x27}; //vendor, dns, domain, fully qualified domain name.
...
int dhcp6_pkt6_request(unsigned char *dhcp, int dhcp_len, char *request, int *reqlen){
    unsigned char msg_type=0x03;
    unsigned short option_code=0;
    unsigned short option_len=0;
    int len=0;
...
int relay_pkt6_request(unsigned char *dhcp, int dhcp_len, char *request, int *reqlen){
    ...
    memcpy(request+len, &dhc6->hopcount, 1); len += 1;
    memcpy(request+len, dhc6->link_addr, 16); len += 16;
    memcpy(request+len, dhc6->peer_addr, 16); len += 16;
...
//   len+=ElapseTime_len;
    ...
    dhcp6_pkt6_request((unsigned char *)*value, ret, dhcp_request, &dhcp_request_len);
...
int pkt6_relay_solicit(char *sip, char *linkaddr, char *client_mac, char *payload, int *payload_len)
{
    unsigned char msg_type=0x0c;
...
int solicit(char *duid, char *ifname, char *pkt, char *linkaddr)
{
    ...
    memcpy(pkt, data_eth, ETH_HDRLEN);        offset += ETH_HDRLEN;
    memcpy(pkt+offset, &iphdr, IP6_HDRLEN);    offset += IP6_HDRLEN;
    memcpy(pkt+offset, &udphdr, UDP_HDRLEN);   offset += UDP_HDRLEN;
    memcpy(pkt+offset, payload, payload_len); offset += payload_len;

    return offset;
}

int relay_solicit(char *sip, char *dip, char *linkaddr, char *client_mac, char *ifname, char *pkt)
{
    int offset = 0;
    char payload[1024] = {0, };
    int payload_len = sizeof(payload);
    ...
    pkt6_udp(546, 547, (char *)&udphdr, (unsigned char *)payload, payload_len, iphdr);
...
int request(nio_pkt_t *adv, char *pkt)
{
    ...
    pkt6_request((unsigned char *)adv->data+62, data_len, (char *)payload, &payload_len);
    tmp = sip;
    for(i=0; i<16; i+=2){
...
int relay_request(nio_pkt_t *adv, char *pkt)
{
    ...
    relay_pkt6_request((unsigned char *)adv->data+62, data_len, (char *)payload, &payload_len);
    tmp = sip;
    for(i=0; i<16; i+=2){
        sprintf(tmp, "%02x", *(adv->data+38+i));
        tmp+=2;
