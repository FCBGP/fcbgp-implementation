 ...
 */
int get_option(dhcpv6_t *pkt, int optcode, char **value)
{
    dhcpv6_opt_t pkt_tmp;
    pkt_tmp.code = optcode;
    slist_foreach(pkt->opt_list, get_opt, &pkt_tmp, sizeof(dhcpv6_opt_t));
    *value = pkt_tmp.value;
    return pkt_tmp.len;
}
...
int pkt6_request(unsigned char *dhcp, int dhcp_len, char *request, int *reqlen){
    ...
    memcpy(request+len, tran_id, sizeof(tran_id));
    len+=sizeof(tran_id);
    ...
    ret = get_option(dhc6, 0x01, value);
    ...
    ret = get_option(dhc6, 0x02, value);
...
//   len+=ElapseTime_len;
    ...
    ret = get_option(dhc6, 0x03, value);
...
int relay_pkt6_request(unsigned char *dhcp, int dhcp_len, char *request, int *reqlen){
    ...
    dhcpv6_t *dhc6;
    dhc6 = parse_dhcp(dhcp+34, dhcp_len);
    if (dhc6 == NULL) {
        DIAG_ERROR("parse dhcp failed.\n");
        return -1;
    }
    ...
    /* interface id */
    ret = get_option(dhc6, 0x12, value);
...
//   len+=ElapseTime_len;
    ...
    ret = get_option(dhc6, 0x09, value);
    option_code = htons(0x09);
    memcpy(request+len, &option_code, sizeof(option_code));
    len += sizeof(option_code);

    char dhcp_request[1024] = {0};
    int dhcp_request_len = 0;
    pkt6_request((unsigned char *)*value, ret, dhcp_request, &dhcp_request_len);
