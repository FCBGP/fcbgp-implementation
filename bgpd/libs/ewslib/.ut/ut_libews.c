...
int ExampleHandler(struct mg_connection *conn, void *cbdata)
{
    ...
    mg_printf(conn, "<p>To see a page from the close handler <a href=\"close\">click close</a></p>");
    mg_printf(conn,
        "<p>To see a page from the FileHandler handler <a "
        "href=\"form\">click form</a> (the starting point of the <b>form</b> test)</p>");
    mg_printf(conn, "<p>To see a page from the CookieHandler handler <a href=\"cookie\">click cookie</a></p>");
    mg_printf(conn,
        "<p>To see a page from the PostResponser handler <a href=\"postresponse\">click post response</a></p>");
    mg_printf(conn,
        "<p>To see an example for parsing files on the fly <a "
        "href=\"on_the_fly_form\">click form</a> (form for uploading files)</p>");
...
int CloseHandler(struct mg_connection *conn, void *cbdata)
{
    /* Handler may access the request info using mg_get_request_info */
    //const struct mg_request_info *req_info = mg_get_request_info(conn);
...
int FormHandler(struct mg_connection *conn, void *cbdata)
{
    /* Handler may access the request info using mg_get_request_info */
    const struct mg_request_info *req_info = mg_get_request_info(conn);
    int ret;
...
int FileUploadForm(struct mg_connection *conn, void *cbdata)
{
    mg_printf(conn, "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n");
...
int field_disp_read_on_the_fly(const char *key, const char *filename, char *path, size_t pathlen, void *user_data)
{
...
int field_get_checksum(const char *key, const char *value, size_t valuelen, void *user_data)
{
    struct tfiles_checksums *context = (struct tfiles_checksums *) user_data;
    (void) key;

    context->file[context->index - 1].length += valuelen;
    md5_append(&(context->file[context->index - 1].chksum), (const md5_byte_t *) value, valuelen);

    return 0;
}


int CheckSumHandler(struct mg_connection *conn, void *cbdata)
{
    /* Handler may access the request info using mg_get_request_info */
    const struct mg_request_info *req_info = mg_get_request_info(conn);
    int i, j, ret;
...
int CookieHandler(struct mg_connection *conn, void *cbdata)
{
    /* Handler may access the request info using mg_get_request_info */
    const struct mg_request_info *req_info = mg_get_request_info(conn);
...
#ifdef USE_SSL_DH
    /* example from https://github.com/ews/civetweb/issues/347 */
    DH *dh = get_dh2236();
    if (!dh)
        return -1;
...
/* Check if libews has been built with all required features. */
#ifdef USE_IPV6
    if (!mg_check_feature(8)) {
        fprintf(stderr, "Error: Embedded example built with IPv6 support, but libews build without.\n");
        err = 1;
    }
#endif
#ifdef USE_WEBSOCKET
    if (!mg_check_feature(16)) {
        fprintf(stderr,
            "Error: Embedded example built with websocket support, but libews build without.\n");
        err = 1;
    }
#endif
#ifndef NO_SSL
    if (!mg_check_feature(2)) {
        fprintf(stderr, "Error: Embedded example built with SSL support, but libews build without.\n");
        err = 1;
    }
#endif
    if (err) {
        fprintf(stderr, "Cannot start Webd - inconsistent build.\n");
        return EXIT_FAILURE;
    }

    /* Start Webd web server */
    ...
    /* Check return value: */
    if (ctx == NULL) {
        fprintf(stderr, "Cannot start Webd - mg_start failed.\n");
        return EXIT_FAILURE;
    }
    ...
    mg_set_websocket_handler(ctx,
        "/websocket", WebSocketConnectHandler, WebSocketReadyHandler, WebsocketDataHandler, WebSocketCloseHandler, 0);
#endif
