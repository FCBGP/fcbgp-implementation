...
rep_ctx_t *rep_create(char *name, int workers, char *address)
{
    ...
    int reuseaddr = 1;
    ret = setsockopt(ctx->fd, SOL_SOCKET, SO_REUSEADDR, &reuseaddr, sizeof(reuseaddr));
    if (ret < 0) {
    ...
    ret = bind(ctx->fd, (struct sockaddr *)&ctx->sockaddr, sizeof(struct sockaddr_un));
    if (ret != 0) {
...
int rep_sendmsg(rep_worker_t *worker)
{
    ...
    ret = sock_sendmsg(worker->cid, stream_address(worker->output), stream_length(worker->output));
...
int rep_start(rep_ctx_t *ctx)
{
    ...
    listenerset_add(ctx->listenerset, ctx->fd, rep_event_handler, LISTEN_EVENT_READ, ctx, 0);
    return 0;
}
