...
#ifdef USE_FC
/* Parse FC attribute information */
                       ...
                       char *fcbuff)
{
    ...
    stream_get(fcbuff, BGP_INPUT(peer), length);
    ...
    while (curpos < length)
    {
        ...
        memcpy(&fc->previous_asn, &fcbuff[curpos], sizeof(u32));
        curpos += sizeof(u32);
        fc->previous_asn = ntohl(fc->previous_asn);

        memcpy(&fc->current_asn, &fcbuff[curpos], sizeof(u32));
        curpos += sizeof(u32);
        fc->current_asn = ntohl(fc->current_asn);

        memcpy(&fc->nexthop_asn, &fcbuff[curpos], sizeof(u32));
        curpos += sizeof(u32);
        fc->nexthop_asn = ntohl(fc->nexthop_asn);

        memcpy(&fc->ski, &fcbuff[curpos], FC_SKI_LENGTH);
        curpos += FC_SKI_LENGTH;

        fc->algo_id = fcbuff[curpos];
        curpos += sizeof(u8);

        fc->flags = fcbuff[curpos];
        curpos += sizeof(u8);

        memcpy(&fc->siglen, &fcbuff[curpos], sizeof(u16));
        curpos += sizeof(u16);
        fc->siglen = ntohs(fc->siglen);
        memcpy(fc->sig, &fcbuff[curpos], fc->siglen);
        curpos += fc->siglen;
...
#ifdef USE_FC
    /* FC_BGP: Add FC path attr. to BGP-UPDATE. */
    if (prefix_for_fc)
    {
